...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    res = temperature * neutrons_emitted
    return temperature < 800 and neutrons_emitted > 500 and res < 500000
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percent_val = (gen_power / theoretical_max_power) * 100

    if percent_val >= 80:
        return "green"
    elif percent_val >= 60 and percent_val < 80:
        return "orange"
    elif percent_val >= 30 and percent_val < 60:
        return "red"
    else:
        return "black"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    res = round(temperature * neutrons_produced_per_second, 0)

    threshold_ten = round(threshold * 0.1, 0)
    # threshold must be between 10% of low end or 10% of high end
    threshold_low = threshold - threshold_ten
    threshold_high = threshold + threshold_ten
    

    if res < round(threshold * 0.90, 0):
        return "LOW"
    elif res >= threshold_low and res <= threshold_high:
        return "NORMAL"
    else:
        return "DANGER"
